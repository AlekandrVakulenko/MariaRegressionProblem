...
if (maxindex>5)&&(maxindex<numel(C_Temp2)-5) && C_min2(maxindex-5)<C_min2(maxindex) && C_min2(maxindex+5)<C_min2(maxindex)
    TempMaxADD = ((TempMax-212)/20)^2;
else
    TempMaxADD = 1000;
end
...
MaxValueADD = (abs(max(abs(C_min2))-2750)>2000)*1000; %STEP
...
out = sum([(B_min-B_exp)./weg(B_exp)*1.4 (q_min-q_exp)./weg(q_exp)*1 (C_min-C_exp)./weg(C_exp).*C_weight ].^2) ...
      + TempMaxADD*100 ...
      + MaxValueADD;
